...
function setup()
{
    ...
    //alterar o tamanho
    tRex.scale = 0.5
    //alterar um posição eseoecífica
    tRex.x = 50;

    //configurações o chão
    chao = createSprite(100, 130, 100, 10);
    chao.addImage("chao", chaoImg);
    chaoInisivel = createSprite(80, 150, 100, 20);
    chaoInisivel.visible = false;
...
//função que executa enquanto o código estuver funcionando
function draw() 
{
    background("#BFBFBF");
...
function criaCactos()
{
    if(frameCount % 60 === 0)
    {
        var cacto = createSprite(800,122,10,80);
        cacto.velocityX = -2;
